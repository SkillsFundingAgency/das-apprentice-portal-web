...
namespace SFA.DAS.ApprenticePortal.UnitTests
{
    ...
    {
        public static T GetOrAdd<T>(this FeatureContext feature, [CallerMemberName] string name = "")
            where T : class, new()
            => GetOrAdd(feature, () => new T(), name);

        public static T GetOrAdd<T>(this FeatureContext feature, Func<T> creator, [CallerMemberName] string name = "")
            where T : class
        {
